...
# Load the data, ignoring bad lines
data = pd.read_csv('data.csv', encoding='ISO-8859-1', on_bad_lines='skip')

# Standardize 'Service Provider 1' column: remove extra spaces, make lowercase, and replace specific names
data['Service Provider 1'] = data['Service Provider 1'].str.strip().str.lower()
data['Service Provider 1'] = data['Service Provider 1'].replace({
    'vodafone in': 'vi',
    'vi india': 'vi'
})

# Plot the Pie Chart of Service Providers
...
plt.title('Service Provider Distribution', fontsize=14, pad=20)
...
data = pd.read_csv('data.csv', encoding='ISO-8859-1', on_bad_lines='skip')
# Standardize 'Manufacturer/Parent Company of your Mobile' columns
data['Manufacturer/Parent Company of your Mobile'] = data['Manufacturer/Parent Company of your Mobile'].str.strip().str.lower()
data['Manufacturer/Parent Company of your Mobile'] = data['Manufacturer/Parent Company of your Mobile'].replace({
...
# Convert positive signal strength values to negative in relevant columns
signal_cols = [col for col in data.columns if 'Signal Strength for Place' in col]
data[signal_cols] = data[signal_cols].apply(pd.to_numeric, errors='coerce').applymap(lambda x: -abs(x) if pd.notnull(x) and x > 0 else x)

# Categorize signal strengths
def categorize_signal(signal):
    if pd.isnull(signal):  # Handles NaN cases
        return None
    if signal <= -110:
        return 'Weak'
    elif -110 < signal <= -90:
...
# Apply the categorization function to all signal columns and reshape data for grouping
data_melted = data.melt(id_vars=['Service Provider 1'], value_vars=signal_cols, var_name='Place', value_name='Signal Strength')
data_melted['Signal Category'] = data_melted['Signal Strength'].apply(categorize_signal)

# Standardize service provider names
data_melted['Service Provider 1'] = data_melted['Service Provider 1'].str.strip().str.lower()
data_melted['Service Provider 1'] = data_melted['Service Provider 1'].replace({
    'vodafone in': 'vi',
    'vi india': 'vi'
})

# Count occurrences of each signal category by service provider
signal_counts = data_melted.groupby(['Service Provider 1', 'Signal Category']).size().unstack(fill_value=0)

# Plot the stacked bar chart
plt.figure(figsize=(10, 6))
signal_counts.plot(kind='bar', stacked=True, color=['#ff9999', '#66b3ff', '#99ff99'])
plt.title('Signal Strength Categories by Service Provider', fontsize=16, pad=15)
plt.xlabel('Service Provider', fontsize=12)
plt.ylabel('Count', fontsize=12)
plt.xticks(rotation=45)
plt.legend(title='Signal Strength', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.show()
...
# Extract numeric part from signal strength values and convert to float
data['Signal Strength for Place 1'] = pd.to_numeric(data['Signal Strength for Place 1'].str.extract(r'(-?\d+)')[0], errors='coerce')

# Standardize 'Service Provider 1' and 'Manufacturer/Parent Company of your Mobile' columns
data['Service Provider 1'] = data['Service Provider 1'].str.strip().str.lower()
data['Service Provider 1'] = data['Service Provider 1'].replace({
    'vodafone in': 'vi',
    'vi india': 'vi'
...
"""# 5.11: Signal Strength Comparison across manufacturers
...
# You might need to inspect the column names using data.columns
actual_column_name = 'Manufacturer/Parent Company of your Mobile'

# Replace 'manufacturer_parent_company_of_your_mobile' with the actual column name
unique_companies = data[actual_column_name].unique()

# Set up the subplots
fig, axs = plt.subplots(len(unique_companies), 1, figsize=(12, 6 * len(unique_companies)))
fig.tight_layout(pad=5.0)

# Iterate over each manufacturer company and create a plot
for i, company in enumerate(unique_companies):
    filtered_data = data[data[actual_column_name] == company]
    ...
    for provider in filtered_data['Service Provider 1'].unique():
        provider_data = filtered_data[filtered_data['Service Provider 1'] == provider]

        # Check if provider_data is empty before accessing values
        signal_strength = []
        if not provider_data.empty:
            signal_strength = [provider_data[f'Signal Strength for Place {i}'].values[0] if len(provider_data[f'Signal Strength for Place {i}'].values) > 0 else None for i in range(1, 11)]
            signal_strength = [x for x in signal_strength if x is not None]  # Remove None values


        # Plot each provider's data if signal_strength is not empty
        if signal_strength:
            ax.plot(range(1, len(signal_strength) + 1), signal_strength, marker='o', label=f'Provider: {provider}')
